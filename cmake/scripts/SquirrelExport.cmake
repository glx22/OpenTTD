...
foreach(LINE IN LISTS SOURCE_LINES)
        ...
        if((DEFINED CLS_PARAMS OR DEFINED METHODS) AND NOT "${SUPER_CLS}" MATCHES "^ScriptEvent" AND NOT "${CLS}" STREQUAL "ScriptEvent")
            if(CLS_SUSPENDABLE)
                if("${CLS_TYPES}" STREQUAL "v")
                    string(APPEND SQUIRREL_EXPORT "\n\tSQ${API_CLS}.AddSQAdvancedConstructor<void (${CLS}::*)()>(engine, &${CLS}::Constructor);")
                else()
                    string(APPEND SQUIRREL_EXPORT "\n\tSQ${API_CLS}.AddConstructor<void (${CLS}::*)()>(engine, &${CLS}::Constructor, \"${CLS_TYPES}\");")
                endif()
            else()
                if("${CLS_TYPES}" STREQUAL "v")
    ...
    if("${LINE}" MATCHES "^.*\\(.*\\).*$")
        ...
        if(("${FUNCTYPE}" STREQUAL "${CLS}" AND NOT FUNCNAME) OR "${FUNCNAME}" STREQUAL "Constructor")
            if(DEFINED API_SELECTED)
