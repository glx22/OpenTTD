...
static void SaveVersionInConfig(IniFile *ini)
{
	...
	std::string ini_version = std::to_string(INIFILE_VERSION);

	const char * const versions[][3] = {
		{ "version_string", _openttd_revision },
		{ "version_number", version },
		{ "ini_version", ini_version.c_str() },
	};
...
{
	IniFile *generic_ini = IniLoadConfig();
	IniFile *private_ini = new IniFile();
	IniFile *secrets_ini = new IniFile();
	private_ini->LoadFromDisk(_private_file, NO_DIRECTORY);
	secrets_ini->LoadFromDisk(_secrets_file, NO_DIRECTORY);
