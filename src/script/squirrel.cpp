...
void Squirrel::AddMethod(std::string_view method_name, SQFUNCTION proc, std::string_view params, void *userdata, int size, bool suspendable)
{
	...
	sq_pushstring(this->vm, fmt::format("{}{}", method_name, suspendable ? "Suspendable" : ""));
	...
	/* Temporary switch back to default print function. */
	AutoRestoreBackup<SQPrintFunc *> print_func(this->print_func, nullptr);
	...
	std::string squirrel_script = fmt::format(
		...
		"    while(this.{0}Suspendable.acall(args));\n"
		"}}\n"
		"return {0};\n", method_name);
