...
static const NWidgetPart _nested_client_list_widgets[] = {
	...
	EndContainer(),
	NWidget(WWT_PANEL, COLOUR_GREY), SetPIP(2, 2, 0),
		NWidget(NWID_SELECTION, INVALID_COLOUR, WID_CL_SERVER_SELECTOR),
			NWidget(WWT_FRAME, COLOUR_GREY), SetDataTip(STR_NETWORK_CLIENT_LIST_SERVER, STR_NULL), SetPadding(0, 4, 0, 4), SetPIP(0, 2, 0),
			...
			EndContainer(),
		EndContainer(),
		NWidget(WWT_FRAME, COLOUR_GREY), SetDataTip(STR_NETWORK_CLIENT_LIST_PLAYER, STR_NULL), SetPadding(0, 4, 0, 4), SetPIP(0, 2, 0),
			NWidget(NWID_HORIZONTAL), SetPIP(0, 3, 0),
...
private:
	...
	void RebuildList()
	{
		...
		for (const Company *c : Company::Iterate()) {
			ButtonCommon *chat_button = new Button(SPR_CHAT, STR_NETWORK_CLIENT_LIST_CHAT_COMPANY_TOOLTIP, COLOUR_ORANGE, c->index, &NetworkClientListWindow::OnClickCompanyChat);

			this->buttons[line_count].emplace_back(chat_button);
			if (own_ci->client_playas != c->index) this->buttons[line_count].emplace_back(new Button(SPR_JOIN, STR_NETWORK_CLIENT_LIST_JOIN_TOOLTIP, COLOUR_ORANGE, c->index, &NetworkClientListWindow::OnClickCompanyJoin));

			this->line_count += 1;

			bool has_players = false;
			for (const NetworkClientInfo *ci : NetworkClientInfo::Iterate()) {
				if (ci->client_playas != c->index) continue;
				if (StrEmpty(ci->client_name)) continue;
				has_players = true;

				if (_network_own_client_id != ci->client_id) this->buttons[line_count].emplace_back(new Button(SPR_CHAT, STR_NETWORK_CLIENT_LIST_CHAT_CLIENT_TOOLTIP, COLOUR_ORANGE, ci->client_id, &NetworkClientListWindow::OnClickClientChat));
				if (_network_server && _network_own_client_id != ci->client_id) this->buttons[line_count].emplace_back(new Button(SPR_ADMIN, STR_NETWORK_CLIENT_LIST_ADMIN_TOOLTIP, COLOUR_RED, ci->client_id, &NetworkClientListWindow::OnClickClientAdmin));
		...
		/* Spectators */
		ButtonCommon *chat_button = new Button(SPR_CHAT, STR_NETWORK_CLIENT_LIST_CHAT_SPECTATOR_TOOLTIP, COLOUR_ORANGE, COMPANY_SPECTATOR, &NetworkClientListWindow::OnClickCompanyChat);
		this->buttons[line_count].emplace_back(chat_button);
		if (own_ci->client_playas != COMPANY_SPECTATOR) this->buttons[line_count].emplace_back(new Button(SPR_JOIN, STR_NETWORK_CLIENT_LIST_JOIN_TOOLTIP, COLOUR_ORANGE, COMPANY_SPECTATOR, &NetworkClientListWindow::OnClickCompanyJoin));

		this->line_count += 1;

		bool has_players = false;
		for (const NetworkClientInfo *ci : NetworkClientInfo::Iterate()) {
			if (ci->client_playas != COMPANY_SPECTATOR) continue;
			if (StrEmpty(ci->client_name)) continue;
			has_players = true;
	...
	ButtonCommon *GetButtonAtPoint(Point pt)
	{
		...
		bool rtl = _current_text_dir == TD_RTL;
		uint x = rtl ? (uint)WD_FRAMERECT_LEFT : widget_matrix->current_x - WD_FRAMERECT_RIGHT;
		...
		for (auto &button : button_find->second) {
			...
			uint width = button->width + WD_FRAMERECT_LEFT + WD_FRAMERECT_RIGHT;
			x += rtl ? width : -width;
		}
...
public:
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		switch (widget) {
			case WID_CL_SERVER_NAME_EDIT:
				...
				SetDParamStr(0, _settings_client.network.server_name);
				ShowQueryString(STR_JUST_RAW_STRING, STR_NETWORK_CLIENT_LIST_SERVER_NAME_QUERY_CAPTION, NETWORK_NAME_LENGTH, this, CS_ALPHANUMERAL, QSF_ENABLE_DEFAULT | QSF_LEN_IN_CHARS);
				break;

			case WID_CL_CLIENT_NAME_EDIT:
				this->query_widget = WID_CL_CLIENT_NAME_EDIT;
				SetDParamStr(0, _settings_client.network.client_name);
				ShowQueryString(STR_JUST_RAW_STRING, STR_NETWORK_CLIENT_LIST_PLAYER_NAME_QUERY_CAPTION, NETWORK_CLIENT_NAME_LENGTH, this, CS_ALPHANUMERAL, QSF_ENABLE_DEFAULT | QSF_LEN_IN_CHARS);
				break;
	...
	void OnQueryTextFinished(char *str) override
	{
		...
		switch (this->query_widget) {
			...
			case WID_CL_SERVER_NAME_EDIT: {
				...
				SetSettingValue(index, StrEmpty(str) ? "My Server" : str);
				this->InvalidateData();
				break;
			}
			...
			case WID_CL_CLIENT_NAME_EDIT: {
				...
				SetSettingValue(index, StrEmpty(str) ? "Player" : str);
				this->InvalidateData();
				break;
			}
	...
	uint DrawButtons(uint x, uint y, const std::vector<std::unique_ptr<ButtonCommon>> *buttons) const
	{
	...
	uint DrawCompany(const Company *c, uint left, uint right, uint top, uint count) const
	{
		bool rtl = _current_text_dir == TD_RTL;
		...
		uint count_start = this->vscroll->GetPosition();
		uint count_end = count_start + this->vscroll->GetCapacity();

		uint y = top + (this->line_height * (count - count_start));
		...
		if (count >= count_start && count < count_end) {
			...
			auto button_find = this->buttons.find(count);
			if (button_find != this->buttons.end()) {
				x = this->DrawButtons(x, y, &button_find->second);
			}

			if (c == nullptr) {
				DrawSprite(SPR_COMPANY_ICON, PALETTE_TO_GREY, rtl ? right - d.width - 2 : left + 2, y + offset);
				DrawString(rtl ? x : text_left, rtl ? text_right : x, y + text_y_offset, STR_NETWORK_CLIENT_LIST_SPECTATORS, TC_SILVER);
			} else {
				DrawCompanyIcon(c->index, rtl ? right - d.width - 2 : left + 2, y + offset);

				SetDParam(0, c->index);
				SetDParam(1, c->index);
				DrawString(rtl ? x : text_left, rtl ? text_right : x, y + text_y_offset, STR_COMPANY_NAME_COMPANY_NUM, TC_SILVER);
			}
		}
	...
	void DrawWidget(const Rect &r, int widget) const override
	{
		switch (widget) {
			case WID_CL_MATRIX: {
				...
				for (const Company *c : Company::Iterate()) {
					count = this->DrawCompany(c, r.left, r.right, r.top, count);
				}
				/* Specators */
				count = this->DrawCompany(nullptr, r.left, r.right, r.top, count);
			} break;
		}
	}
};
